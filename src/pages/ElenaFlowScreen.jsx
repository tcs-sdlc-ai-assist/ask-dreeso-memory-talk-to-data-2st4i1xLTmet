    ...
    >
      <div className="flex items-start justify-between gap-2 mb-2">
        ...
        <div
          ...
          style={{ backgroundColor: stat.color ? `${stat.color}20` : 'rgba(16,185,129,0.12)' }}
        >
          <StatIcon
            icon={stat.icon}
            className="w-4 h-4"
...
function LoadingSpinner() {
  return (
    <svg
      className="w-8 h-8 animate-spin text-accent-green"
      viewBox="0 0 24 24"
      fill="none"
      aria-hidden="true"
    >
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 px-4 animate-fade-in">
      ...
      <p className="text-xs text-slate-500">
        Analyzing financial data across enterprise systems
      </p>
...
/**
 * Screen 7: Commercial & Procurement overview with query input and quick stats
 * @param {Object} props
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <GlassCard variant="elevated" className="!p-5 sm:!p-6">
        <h2 className="text-base sm:text-lg font-semibold text-slate-100 mb-4">
          Ask anything about financials & commercial
        </h2>
        <QueryInput
          onSubmit={onQuerySubmit}
          isLoading={isLoading}
          persona={persona}
          placeholder="e.g., Show me revenue analysis, What are the budget variances?"
        />
      </GlassCard>
      ...
      {!activeCluster && defaultResult && (
        <div className="animate-fade-in" style={{ animationDelay: '200ms', animationFillMode: 'both' }}>
          <ForecastChart
            ...
            subtitle="Key financial metrics across your active portfolio"
          />
        </div>
      )}
    </div>
...
/**
 * Screen 8-9: Result display screen with structured output
 * @param {Object} props
...
/**
 * Screen 10: Action execution and confirmation screen
 * @param {Object} props
...
/**
 * Elena persona flow screen component (Screens 7-10).
 * Screen 7: Commercial & Procurement overview with query input.
 * Screen 8: Cost variance and risk analysis results.
 * Screen 9: Subcontractor performance analytics.
 * Screen 10: Commercial action execution (SAP + Procore).
 * Each screen supports all view states (query, loading, result, CTA, action, confirmation).
 *
 * @param {Object} props
 * @param {string} [props.className] - Additional class names for the wrapper
 * @returns {React.ReactElement} The Elena flow screen component
 */
export function ElenaFlowScreen({ className }) {
  ...
  const currentPersona = persona || PERSONAS.ELENA.id;
  const currentRole = role || PERSONAS.ELENA.role;
  const currentCluster = cluster || PERSONAS.ELENA.cluster;
  ...
  const renderCurrentView = () => {
    switch (viewState) {
      ...
      case VIEW_STATES.LOADING:
        return <QueryLoadingState message="Analyzing financial data..." />;
            ...
            <GlassCard variant="elevated" className="!p-6 sm:!p-8">
              <div className="flex flex-col items-center text-center">
                ...
                <p className="text-sm text-slate-300 leading-relaxed mb-6 max-w-md">
                  Your commercial action has been executed and logged. You can continue querying or return to the dashboard.
                </p>
          ...
          <div className="px-1 py-3 animate-fade-in">
            <button
              ...
              className={classNames(
                ...
                'focus:outline-none focus:text-accent-green'
              )}
              aria-label="Back to dashboard"
            >
              <svg
...
ElenaFlowScreen.propTypes = {
  className: PropTypes.string,
};

export default ElenaFlowScreen;