...
import { getDashboardStats, getIntelligenceClustersForPersona, INTELLIGENCE_CLUSTERS } from '../data/mockData.js';
...
/**
 * Cross-persona summary KPI data
 * @type {Array<Object>}
 */
const CROSS_PERSONA_KPIS = [
  ...
  { label: 'Revenue YTD', value: 142.5, unit: 'M', trend: 'up', change: 8.3 },
...
/**
 * Cross-persona quick stats
 * @type {Array<Object>}
 */
const CROSS_PERSONA_STATS = [
  { label: 'Total Portfolio', value: 312.8, unit: 'M', icon: 'chart', color: '#06B6D4' },
  { label: 'Active Projects', value: 24, unit: '', icon: 'folder', color: '#3B82F6' },
  { label: 'At Risk', value: 5, unit: '', icon: 'alert', color: '#EF4444' },
    ...
    >
      <div className="flex items-start justify-between gap-2 mb-2">
        ...
        <div
          ...
          style={{ backgroundColor: stat.color ? `${stat.color}20` : 'rgba(6,182,212,0.12)' }}
        >
          <StatIcon
            icon={stat.icon}
            className="w-4 h-4"
...
function LoadingSpinner() {
  return (
    <svg
      className="w-8 h-8 animate-spin text-accent-cyan"
      viewBox="0 0 24 24"
      fill="none"
      aria-hidden="true"
    >
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 px-4 animate-fade-in">
      ...
      <p className="text-xs text-slate-500">
        Analyzing cross-domain data across all enterprise systems
      </p>
...
/**
 * Cross-system summary card component
 * @returns {React.ReactElement}
 */
function CrossSystemSummary() {
  const systems = [
    { name: 'SAP ERP', status: 'live', color: '#0070F2', latency: '120ms' },
    { name: 'Procore', status: 'live', color: '#F47E20', latency: '85ms' },
    { name: 'Salesforce CRM', status: 'live', color: '#00A1E0', latency: '200ms' },
    { name: 'Oracle Primavera', status: 'live', color: '#C74634', latency: '310ms' },
  ];
    ...
    <GlassCard className="!p-4 sm:!p-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        {systems.map((system, index) => (
          ...
          >
            <span
              className="block w-2 h-2 rounded-full bg-accent-green shadow-[0_0_6px_rgba(16,185,129,0.6)]"
              aria-hidden="true"
            />
            <div className="min-w-0">
              <p className="text-xs font-medium text-slate-200 truncate">{system.name}</p>
              <p className="text-xs text-slate-500">{system.latency}</p>
            </div>
          </div>
        ))}
      </div>
...
/**
 * Screen 19: Cross-persona dashboard overview with query input and all clusters
 * @param {Object} props
...
function DashboardScreen({ onQuerySubmit, isLoading, persona, cluster }) {
  const allClusters = useMemo(() => INTELLIGENCE_CLUSTERS, []);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
        {CROSS_PERSONA_STATS.map((stat, index) => (
      ...
      <GlassCard variant="elevated" className="!p-5 sm:!p-6">
        <h2 className="text-base sm:text-lg font-semibold text-slate-100 mb-1">
          Cross-Domain Intelligence
        </h2>
        <p className="text-xs sm:text-sm text-slate-400 mb-4">
          Ask questions spanning operations, finance, engineering, and sales
        </p>
        <QueryInput
          onSubmit={onQuerySubmit}
          isLoading={isLoading}
          persona={persona}
          placeholder="e.g., Give me a portfolio overview, Show me cross-project risks, Revenue forecast..."
        />
      </GlassCard>

      {/* Cross-Persona KPI Overview */}
      <div className="animate-fade-in" style={{ animationDelay: '150ms', animationFillMode: 'both' }}>
        <ForecastChart
          data={CROSS_PERSONA_KPIS}
          chartType="kpi"
          title="Enterprise Performance Summary"
          subtitle="Combined insights across all domains and enterprise systems"
        />
      </div>

      {/* All Intelligence Clusters */}
      {allClusters.length > 0 && (
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            {allClusters.map((clusterObj, index) => (
              <div
                key={clusterObj.id}
...
/**
 * Result display screen with structured output
 * @param {Object} props
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <GlassCard className="!p-4 sm:!p-5">
        <QueryInput
          ...
          isLoading={isLoading}
          persona={persona}
          placeholder="Ask a cross-domain follow-up question..."
        />
      </GlassCard>
...
function inferActionType(template) {
  ...
  if (name.includes('update') || name.includes('export') || name.includes('resolve')) return 'UPDATE';
  return 'UPDATE';
}

/**
 * Action execution and confirmation screen
 * @param {Object} props
...
/**
 * Cross-persona final analytics screen component (Screen 19).
 * Displays combined insights across all personas, cross-system summary,
 * and final action recommendations. Includes query input for cross-domain
 * questions and comprehensive CTA bubbles.
 * Each view supports all view states (query, loading, result, CTA, action, confirmation).
 *
 * @param {Object} props
 * @param {string} [props.className] - Additional class names for the wrapper
 * @returns {React.ReactElement} The final flow screen component
 */
export function FinalFlowScreen({ className }) {
  ...
  const currentRole = role || 'Cross-Domain Analyst';
  const currentCluster = cluster || 'portfolio';
  ...
  const renderCurrentView = () => {
    switch (viewState) {
      ...
      case VIEW_STATES.LOADING:
        return <QueryLoadingState message="Analyzing cross-domain enterprise data..." />;
            ...
            <GlassCard variant="elevated" className="!p-6 sm:!p-8">
              <div className="flex flex-col items-center text-center">
                ...
                <p className="text-sm text-slate-300 leading-relaxed mb-6 max-w-md">
                  Your cross-domain action has been executed and logged across all relevant enterprise systems. You can continue querying or return to the dashboard.
                </p>
        ...
        {viewState !== VIEW_STATES.QUERY_INPUT && (
          <div className="px-1 py-3 animate-fade-in">
            <button
              ...
              aria-label="Back to cross-domain dashboard"
            >
              <svg
              ...
              <span>Back to Cross-Domain Dashboard</span>
            </button>
          </div>
        )}
...
FinalFlowScreen.propTypes = {
  className: PropTypes.string,
};

export default FinalFlowScreen;